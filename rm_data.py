...
def get_data(verbose=True):
    ...
    def preprocess_data_for_reward_model(data, stack_size=4, obs_dim=24, action_dim=20):
        ...
        for l, r, c in zip(data['left'], data['right'], data['user_choice']):
            ...
            if len(slice_l_obs) > 0 and len(slice_l_action) > 0 and len(slice_r_obs) > 0 and len(slice_r_action) >0: # sometimes there is traj error (len=0)
...
with open(f'processed_data/itr{iter}/processed_data_{frame_number}.pkl', 'rb') as f:
